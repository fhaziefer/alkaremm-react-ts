...
import generasiName from '../../JSON/generasiName.json'
...
const SearchScreen = () => {
  ...
  const handleBani = (event: any) => {
    const selectedBani = event.value.toLowerCase()
    setBaniQuery(selectedBani);
    setPage(1)
  }

  const handleGenerasi = (event: any) => {
    const generasiSelected = event.value.toLowerCase()
    setGenerasiQuery(generasiSelected);
    setPage(1)
  }
  ...
  }, [debouncedQuery, generasiQuery, baniQuery, page, role]);
    ...
    <div className="flex flex-col min-h-screen items-center pt-4">
      ...
      {(role === 'ADMIN' || role === 'USER') && (
        <>
          ...
          <div className='w-[94%] sm:w-[77%] md:w-[77%] lg:w-[57%] carousel carousel-start rounded-box mt-4'>
            <div onChange={(e) => { handleBani(e.target) }} className="carousel-item gap-2 md:gap-4">
              <input
                value=''
                className="join-item btn btn-sm"
                type="radio"
